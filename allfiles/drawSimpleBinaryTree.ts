...
function recursivelyDrawNodes(
  ...
  if (root.left) {
    ...
    connectPointsWithBezierCurve(canvasComponent,
      xPosition,
      (xStart + xPosition) / 2,
      yPosition + FIXED_VALUE.radius,
      getCanvasHeightFromTreeHeight(currentLine + 1) - FIXED_VALUE.radius,
    );
  }
...
function drawSimpleBinaryTree(
  ...
  if(!isGameTree){
     recursivelyDrawNodes(root, canvasComponent, 1,xStart, xEnd);
      ...
      recursivelyDrawNodes(root, canvasComponent, root.gameYposition,xStart, xEnd);
